...
@synthesize expandingAdjustScrollUnable = _expandingAdjustScrollUnable;
@synthesize closingAdjustScrollUnable = _closingAdjustScrollUnable;
...
- (NSMutableDictionary *)expandableSectionsDictionary
{
    if (!_expandableSectionsDictionary) {
        _expandableSectionsDictionary = [NSMutableDictionary dictionary];
    }
    return _expandableSectionsDictionary;
}

- (NSMutableDictionary *)showingSectionsDictionary
{
    if (!_showingSectionsDictionary) {
        _showingSectionsDictionary = [NSMutableDictionary dictionary];
    }
    return _showingSectionsDictionary;
}

- (NSMutableDictionary *)downloadingSectionsDictionary
{
    if (!_downloadingSectionsDictionary) {
        _downloadingSectionsDictionary = [NSMutableDictionary dictionary];
    }
    return _downloadingSectionsDictionary;
}

- (NSMutableDictionary *)animatingSectionsDictionary
{
    if (!_animatingSectionsDictionary) {
        _animatingSectionsDictionary = [NSMutableDictionary dictionary];
    }
    return _animatingSectionsDictionary;
}
...
- (void)expandSection:(NSInteger)section animated:(BOOL)animated {
    ...
    if ( _lastAnimateTime ) {
        ...
        NSTimeInterval interval = [currentDate timeIntervalSinceDate: _lastAnimateTime];
        
        if ( interval < 0.35f ) {
            animated = NO;
        }
    }
    _lastAnimateTime = [NSDate date];
    ...
    if (_expandingAdjustScrollUnable == NO) {
...
- (void)collapseSection:(NSInteger)section animated:(BOOL)animated {
    ...
    if (animated && newRowCount <= self.maximumRowCountToStillUseAnimationWhileExpanding) {  //FIX BUG 12408
        [self beginUpdates];
    ...
    if (_closingAdjustScrollUnable == NO) {
        [self scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:section]
                    atScrollPosition:UITableViewScrollPositionTop
                            animated:animated];
    }
...
// http://rdc.wochacha.cn/issues/11251 -kun 2013-10-25
- (void)deleteSections:(NSIndexSet *)sections withRowAnimation:(UITableViewRowAnimation)animation
{
	NSUInteger indexOfSection = 0;
	
	NSUInteger numberOfSections = [self.showingSectionsDictionary count];
	while (indexOfSection<numberOfSections) {
		
		NSNumber *key = [NSNumber numberWithInteger:indexOfSection];
		if (indexOfSection<sections.firstIndex) {
//			Do nothing;
		} else if(indexOfSection<=sections.lastIndex) {
			[self.showingSectionsDictionary removeObjectForKey:key];
		} else {
			BOOL value = [[self.showingSectionsDictionary objectForKey:key] boolValue];
			[self.showingSectionsDictionary removeObjectForKey:key];
			key = [NSNumber numberWithInteger:(indexOfSection-sections.count)];
			[self.showingSectionsDictionary setObject:[NSNumber numberWithBool:value] forKey:key];
		}
    ...
    if ([[self.expandableSectionsDictionary objectForKey:key] boolValue]) {
        ...
        if (indexPath.row == 0) {
            ...
            if ([self.myDataSource tableView:self needsToDownloadDataForExpandableSection:indexPath.section]) {
				...
				BOOL expand = [[self.showingSectionsDictionary objectForKey:key] boolValue];
                if (expand) {
                    [self collapseSection:indexPath.section animated:YES];
                } else {
                    [self expandSection:indexPath.section animated:YES];
                }
				if ([self.myDelegate respondsToSelector:@selector(tableView:didSelectSectionNote:expand:)]) {
					[self.myDelegate tableView:self didSelectSectionNote:indexPath expand:expand];
				}
            }
        } else {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if ([self.myDataSource tableView:self canExpandSection:section]) {
        ...
        NSNumber *showing = [self.showingSectionsDictionary objectForKey:key];
        if (!showing && self.myDataSource && [self.myDataSource respondsToSelector:@selector(tableView:isSectionExpand:)]) {
            showing = [NSNumber numberWithBool:[self.myDataSource tableView:self isSectionExpand:section]];
            [self.showingSectionsDictionary setObject:showing forKey:key];
        }
...
- (id)initWithCoder:(NSCoder *)decoder {
    ...
    if ((self = [super initWithCoder:decoder])) {
        ...
        if ([decoder decodeIntegerForKey:@"maximumRowCountToStillUseAnimationWhileExpanding"]) {
            ...
            _maximumRowCountToStillUseAnimationWhileExpanding = NSIntegerMax;
        }
